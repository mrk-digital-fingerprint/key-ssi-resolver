 ...
 * @param {BarMap} options.currentBarMapClone
 * @param {BarMap} options.diffBarMap
 */
function SessionBarMap(options) {
    options = options || {};

    let currentBarMapClone = options.currentBarMapClone;
     ...
     */
    const initialize = () => {
        const barMapProperties = Object.getOwnPropertyNames(currentBarMapClone);
        ...
        for (const propertyName of barMapProperties) {
            if (typeof currentBarMapClone[propertyName] !== 'function') {
                continue;
            }
    ...
    const createProxyMethod = (method) => {
        const proxy = new Proxy(function () {}, {
            apply: (target, thisArg, argumentsList) => {
                ...
                return currentBarMapClone[method].apply(currentBarMapClone, argumentsList);
            }
        })
    ...
    this.addFileEntryProxyHandler = (path, bricks) => {
        currentBarMapClone.addFileEntry(path, bricks);
    ...
    this.appendBricksToFileProxyHandler = (path, bricks) => {
        currentBarMapClone.appendBricksToFile(path, bricks);
    ...
    this.deleteProxyHandler = (path) => {
        currentBarMapClone.delete(path);
    ...
    this.copyProxyHandler = (srcPath, dstPath) => {
        currentBarMapClone.copy(srcPath, dstPath);
    ...
    this.setCurrentBarMapClone = (barMap) => {
        currentBarMapClone = barMap;
    }
    ...
    this.applyDiffsStartingWith = (barMapDiff) => {
        for (let i = 0; i < barMapDiffsStack.length; i++) {
            ...
            currentBarMapClone.applyDiff(barMapDiff)
        }
    }
