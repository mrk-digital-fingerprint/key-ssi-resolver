module.exports = function(archive){
	archive.call = (functionName, ...args) => {
		...
		archive.readFile("/code/api.js", function(err, apiCode){
			if(err){
				return OpenDSUSafeCallback(callback)(createOpenDSUErrorWrapper(`Failed to create read file /code/api.js`, err));
			}
			...
			try{
				...
				return OpenDSUSafeCallback(callback)(createOpenDSUErrorWrapper(`Failed to create eval code in file /code/api.js`, err));
			}
		});
	}
	return archive;
