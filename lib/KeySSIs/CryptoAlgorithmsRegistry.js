...
for (let keySSIType in SSITypes) {
    CryptoAlgorithmsRegistry.prototype.registerSignFunction(SSITypes[keySSIType], 'v0', (data, privateKey) => {
        ...
        return crypto.sign('sha256', data, crypto.convertKeys(privateKey, rawPublicKey).privateKey);
    });
}

for (let keySSIType in SSITypes) {
    CryptoAlgorithmsRegistry.prototype.registerVerifyFunction(SSITypes[keySSIType], 'v0', (privateKey, signature) => {
        const keyGenerator = crypto.createKeyPairGenerator();
        const rawPublicKey = keyGenerator.getPublicKey(privateKey, 'secp256k1');
        const publicKey = crypto.convertKeys(privateKey, rawPublicKey).publicKey;
        return crypto.verify('sha256', publicKey, signature);
    });
}
