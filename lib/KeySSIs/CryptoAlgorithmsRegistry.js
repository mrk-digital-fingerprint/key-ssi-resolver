...
for (let keySSIType in SSITypes) {
    CryptoAlgorithmsRegistry.prototype.registerHashFunction(keySSIType, 'v0', (data) => {
...
for (let keySSIType in SSITypes) {
    CryptoAlgorithmsRegistry.prototype.registerKeyDerivationFunction(keySSIType, 'v0', (password, iterations) => {
        return crypto.deriveKey("aes-256-gcm", password, iterations);
    });
}
for (let keySSIType in SSITypes) {
    CryptoAlgorithmsRegistry.prototype.registerEncryptionFunction(keySSIType, 'v0', (plainData, encryptionKey, options) => {
...
for (let keySSIType in SSITypes) {
    CryptoAlgorithmsRegistry.prototype.registerDecryptionFunction(keySSIType, 'v0', (encryptedData, decryptionKey, authTagLength, options) => {
        const pskEncryption = crypto.createPskEncryption('aes-256-gcm');
...
for (let keySSIType in SSITypes) {
    CryptoAlgorithmsRegistry.prototype.registerEncodingFunction(keySSIType, 'v0', (data) => {
        return crypto.pskBase58Encode(data);
    });
}

for (let keySSIType in SSITypes) {
    CryptoAlgorithmsRegistry.prototype.registerDecodingFunction(keySSIType, 'v0', (data) => {
        return crypto.pskBase58Decode(data);
    });
}

for (let keySSIType in SSITypes) {
    CryptoAlgorithmsRegistry.prototype.registerKeyPairGenerator(keySSIType, 'v0', () => {
        return crypto.createKeyPairGenerator();
    });
}

for (let keySSIType in SSITypes) {
    CryptoAlgorithmsRegistry.prototype.registerSignFunction(keySSIType, 'v0', (data, privateKey) => {
        return crypto.sign('sha256', data, privateKey);
    });
}

for (let keySSIType in SSITypes) {
    CryptoAlgorithmsRegistry.prototype.registerVerifyFunction(keySSIType, 'v0', (data, publicKey, signature) => {
