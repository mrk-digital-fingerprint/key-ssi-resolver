...
const getCryptoFunction = (keySSI, algorithmType, version = 'v0') => {
    const keySSIType = typeof keySSI === "object" ? keySSI.getTypeName() : keySSI;
    const vn = typeof keySSI === "object" ? keySSI.getVn() : version
    const identifier = keySSI === "object" ? keySSI.getIdentifier(true) : keySSI;
    let cryptoFunction;
    try {
        cryptoFunction = algorithms[keySSIType][vn][algorithmType];
    } catch (e) {
        cryptoFunction = defaultAlgorithms[algorithmType];
    }

    if (typeof cryptoFunction === "undefined") {
        throw Error(`Algorithm type <${algorithmType}> not recognized for <${identifier}>`);
    }
