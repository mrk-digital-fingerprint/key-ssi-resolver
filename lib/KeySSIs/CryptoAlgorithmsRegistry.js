...
CryptoAlgorithmsRegistry.prototype.getEncryptionKeyGenerationFunction = (keySSI) => {
    return algorithms[keySSI.getName()][keySSI.getVn()].encryptionKeyGeneration;
};
...
for (let keySSIType in SSITypes) {
    CryptoAlgorithmsRegistry.prototype.registerEncryptionKeyGenerationFunction(SSITypes[keySSIType], 'v0', () => {
...
for (let keySSIType in SSITypes) {
    CryptoAlgorithmsRegistry.prototype.registerVerifyFunction(SSITypes[keySSIType], 'v0', (data, privateKey, signature) => {
        const keyGenerator = crypto.createKeyPairGenerator();
        const rawPublicKey = keyGenerator.getPublicKey(privateKey, 'secp256k1');
