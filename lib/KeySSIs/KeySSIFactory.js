...
**/
KeySSIFactory.prototype.create = (identifierOrTypeName) => {
    let typeName;
    if (typeof identifierOrTypeName === "undefined") {
        throw Error("An identifier or keySSI type should be provided");
    }
    if (identifierOrTypeName.includes(":")) {
        const KeySSIMixin = require("./KeySSIMixin");
        KeySSIMixin.autoLoad(identifierOrTypeName);
        typeName = KeySSIMixin.getName();
    } else {
        typeName = identifierOrTypeName;
        identifierOrTypeName = undefined;
    }

    return registry[typeName].functionFactory(identifierOrTypeName);
};
