...
KeySSIFactory.prototype.getRelatedType = (keySSI, otherType, callback) => {
    ...
    let derivedKeySSI;
    try {
        derivedKeySSI = KeySSIFactory.prototype.getDerivedType(keySSI, otherType);
    } catch (err){
        return callback(err);
    }
...
KeySSIFactory.prototype.getDerivedType = (keySSI, derivedTypeName) => {
    if (keySSI.getName() === derivedTypeName) {
        return keySSI;
    }
    let localKeySSI = keySSI;
