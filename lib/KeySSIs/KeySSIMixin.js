...
const { BRICKS_DOMAIN_KEY } = require('./constants')
...
function keySSIMixin(target){
    ...
    const _createHintMap = function(hint = _hint) {
        hint.split(':').forEach((item) => {
            const [key, value] = item.split('=')
            _hintMap[key] = value ? value : true
        })
    }
    ...
    target.getBricksDomain = function() {
        return _hintMap[BRICKS_DOMAIN_KEY] ? _hintMap[BRICKS_DOMAIN_KEY] : _dlDomain;
    }
