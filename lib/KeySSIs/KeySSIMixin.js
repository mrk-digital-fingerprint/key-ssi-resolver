...
function keySSIMixin(target){
    ...
    const _createHintJSON = function(hint = _hint) {
        try {
            _hintJSON = JSON.parse(hint)
        }
        catch (error) {
    ...
    target.getBricksDomain = function() {
        return _hintJSON[BRICKS_DOMAIN_KEY] ? _hintJSON[BRICKS_DOMAIN_KEY] : _dlDomain;
    }
