...
const {CHAINCODE_KEY, NODE_POSITION_KEY, EXPIRED_AT_KEY} = require("./seedSSIConstants");
...
function NodeSSIMixin(target, identifier) {
    ...
    target._deriveChild = function (childNodeFactory, newNodePosition) {
        ...
        childNodeSSI.load(SSITypes.SEED_SSI, target.getDLDomain(), childPrivateKey, '', target.getVn(), JSON.stringify(childHint))
        _childCount++

        return childNodeSSI;
    };

    target.derive = function (createSReadSSIFactory) {
        const sReadSSI = createSReadSSIFactory();
        const privateKey = target.getPrivateKey();
        const sreadSpecificString = cryptoRegistry.getHashFunction(target)(privateKey);
        ...
        sReadSSI.load(SSITypes.SREAD_SSI, target.getDLDomain(), sreadSpecificString, subtypeControl, target.getVn(), target.getHint());
        return sReadSSI;
    };

    // target.deriveLeaf = function (expiredAt) {
    //     const sReasSSI = createSReadSSI();
    ...
    //     }
    //     sReasSSI.load(SSITypes.LEAF_SSI, target.getDLDomain(), '', controlString, target.getVn(), leafHint);
    //     return sReasSSI;
    // };
